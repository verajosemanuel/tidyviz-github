library(devtools)

install_github("AckerDWM/gg3D")
install_github("AliciaSchep/gglabeller")
install_github("ankane/dbx")
install_github("AntoineGuillot2/D3partitionR")
install_github("AppliedDataSciencePartners/xgboostExplainer")
install_github("aryoda/tryCatchLog")
install_github("benmarwick/wordcountaddin", type = "source", dependencies = TRUE)
install_github("bhaskarvk/leaflet.extras")
install_github("blairj09/dockerterm")
install_github("christophM/iml")
install_github("christophsax/tsbox")
install_github("cole-brokamp/rize")
install_github("ColinFay/aside")
install_github("colinfay/dockerfiler")
install_github("ColinFay/fryingpane")
install_github("ColinFay/tidystringdist")
install_github("cttobin/ggthemr")
install_github("dantonnoriega/xmltools")
install_github("daranzolin/compareBars")
install_github("daranzolin/ViewPipeSteps")
install_github("datacamp/RDocumentation")
install_github("dgrtwo/gganimate")
install_github("dmi3kno/hocr")
install_github("dmi3kno/polite")
install_github("dreamRs/addinit")
install_github("dreamRs/prefixer")
install_github("drsimonj/twidlr")
install_github("dyerlab/popgraph")
install_github("edgararuiz/dbplot")
install_github("edgararuiz/tidypredict")
install_github("elbersb/tidylog")
install_github("EmilHvitfeldt/ggpage")
install_github("EmilHvitfeldt/gganonymize")
install_github('emitanaka/anicon')
install_github("gabrielrvsc/HDeconometrics")
install_github("gadenbuie/regexplain")
install_github("gadenbuie/xaringanthemer")
install_github("hadley/precis")
install_github("hafen/trelliscopejs")
install_github("hrbrmstr/cloc")
install_github("hrbrmstr/msgxtractr")
install_github("hrbrmstr/waffle")
install_github("ikosmidis/cranly")
install_github("IndrajeetPatil/ggstatsplot")
install_github("jeremystan/aargh")
install_github("jkaupp/nord")
install_github("johannesbjork/LaCroixColoR")
install_github("kassambara/r2excel")
install_github("kevinushey/Kmisc")
install_github("krlmlr/styler")
install_github("lantanacamara/lightgbmExplainer")
install_github("lbusett/insert_table")
install_github("MangoTheCat/goodpractice")
install_github("MilesMcBain/deplearning")
install_github("MilesMcBain/gistfo")
install_github("MilesMcBain/packup")
install_github("mvuorre/brmstools")
install_github('mwip/beautifyR')
install_github("pbiecek/breakDown")
install_github("reconhub/reportfactory")
install_github("ricardo-bion/ggtech", dependencies=TRUE)
install_github("r-lib/bench")
install_github("r-lib/boxes")
install_github("rich-iannone/blastula")
install_github("rich-iannone/DiagrammeR")
install_github("richarddmorey/tweetRcode", subdir = "tweetRcode")
install_github("richfitz/stevedore", upgrade_dependencies = FALSE)
install_github("ropenscilabs/available")
install_github("ropenscilabs/data-packages")
install_github("ropenscilabs/icon")
install_github("ropenscilabs/packagemetrics")
install_github("ropenscilabs/umapr")
install_github("ropensci/writexl")
install_github("rstats-db/RPostgres")
install_github("rstudio/blogdown")
install_github("seasmith/AlignAssign")
install_github("sicarul/xray")
install_github("smach/rmiscutils")
install_github("Stan125/GREA")
install_github("stephlocke/pRojects")
install_github("STATWORX/bounceR")
install_github("strengejacke/strengejacke")
install_github("swarm-lab/editR") # requires devtools::shinyAce shiny shinyFiles shinyBS rmarkdown knitr
install_github("ThinkR-open/remedy")
install_github("ThinkRstat/littleboxes")
install_github('thomasp85/farver')
install_github("thomasp85/patchwork")
install_github("tjmahr/WrapRmd")
install_github("trinker/regexr")
install_github("trestletech/shinyAce")
install_github("vqv/ggbiplot")
install_github("wilkox/ggfittext")
install_github("WinVector/addinexamples")
install_github("WinVector/rquery")
install_github("yihui/xaringan")
install_github("yutannihilation/mediumr")
install_github("https://gitlab.com/jozefhajnala/jhaddins.git")
install_github("ThinkR-open/testdown")
install_github("rstudio/gt")
